...
class ViewController: UIViewController
{
    ...
    func loadAllUIObjects()
    {
        ...
        for x in data
        {
            ...
            videoUrl.append(configureUrl(urlInString: (x["trailers"] as! [String])[0]))
            print(videoUrl)
            
            //Fetching Audio URL In String
            var tempAudioArray = [String]()
            for k in (x["songs"] as! [String])
            {
            ...
            //Fetching image
            let image = fetchImage(urlInString: (x["posters"] as! [String])[0])
            
            
            
            //Storing All Stories & Images In Singleton object
            MovieDetails.movie.images.append(image)
            MovieDetails.movie.stories.append(x["story"] as? String ?? "STORY NOT AVAILABLE")
           ...
           //Fetching Actor Details
            MovieDetails.movie.actors.append(x["actors"] as! [String])
